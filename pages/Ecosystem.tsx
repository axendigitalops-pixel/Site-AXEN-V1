    ...
    <div className="pt-32 pb-20">
      ...
      <section className="container mx-auto px-6 mb-24 text-center">
        <motion.h1 
          ...
          className="text-5xl md:text-7xl font-display font-bold text-axen-ivory mb-8"
        >
          Nosso Ecossistema
        </motion.h1>
        <p className="text-xl text-axen-gray max-w-3xl mx-auto">
      ...
      <div className="relative h-[400px] flex items-center justify-center mb-32 overflow-hidden">
        ...
        <div className="relative z-10 w-32 h-32 bg-axen-ivory/5 rounded-full blur-[40px]"></div>
        <div className="relative z-20 text-center">
          <span className="text-axen-ivory font-display font-bold text-2xl tracking-wider">NÚCLEO</span>
        </div>
      </div>
        ...
        {arms.map((arm, index) => (
          ...
          >
            <div className="flex-1">
              <div className="w-16 h-16 rounded-full bg-axen-darkgray flex items-center justify-center mb-6 text-axen-ivory border border-white/20 group hover:border-axen-turquoise/30 transition-colors">
                <arm.icon size={32} className="group-hover:text-axen-turquoise transition-colors" />
              </div>
              <h2 className="text-3xl md:text-4xl font-display font-bold text-axen-ivory mb-4">{arm.title}</h2>
              <p className="text-axen-gray text-lg mb-8">{arm.desc}</p>
              
              <ul className="space-y-4">
                {arm.items.map((item, i) => (
                  <li key={i} className="flex items-center gap-3 text-axen-ivory/80">
                    <div className="w-1.5 h-1.5 rounded-full bg-axen-ivory/50"></div>
                    {item}
                  </li>
                ))}
              </ul>
            ...
            <div className="flex-1 w-full">
              <div className="aspect-video rounded-[2rem] bg-gradient-to-br from-axen-darkgray to-black border border-white/5 p-8 flex items-center justify-center relative overflow-hidden group">
      ...
      <section className="container mx-auto px-6 mt-32 max-w-3xl">
        <h2 className="text-3xl font-display font-bold text-axen-ivory mb-12 text-center">Perguntas Frequentes</h2>
        <div className="space-y-4">
          {[
            { q: "A Axen substitui minha equipe interna?", a: "Não. Atuamos como extensão. Nosso núcleo se conecta ao seu time para dar tração e especialidade onde falta." },
            ...
            <details key={idx} className="group bg-axen-darkgray/30 rounded-2xl border border-white/5 overflow-hidden open:border-white/20 transition-colors">
              <summary className="flex justify-between items-center p-6 cursor-pointer font-medium text-axen-ivory list-none">
                {faq.q}
                <ChevronDown className="group-open:rotate-180 transition-transform text-axen-ivory" />
              </summary>
              <div className="px-6 pb-6 text-axen-gray leading-relaxed border-t border-white/5 pt-4">
                {faq.a}
              </div>
            </details>
          ))}
      ...
      <section className="container mx-auto px-6 mt-32 text-center">
        <div className="bg-gradient-to-r from-axen-darkgray to-black p-12 rounded-[3rem] border border-white/10 relative overflow-hidden">
            <div className="absolute top-0 right-0 w-64 h-64 bg-white/5 blur-[80px] rounded-full"></div>
            <h2 className="text-3xl md:text-4xl font-display font-bold text-axen-ivory mb-8 relative z-10">Pronto para integrar seu núcleo?</h2>
            <Link to="/contact">
